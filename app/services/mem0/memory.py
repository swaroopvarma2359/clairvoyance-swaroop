...
class ImprovedMem0MemoryService(Mem0MemoryService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._session_timeout = kwargs.get('session_timeout', config.MEM0_SESSION_TIMEOUT)
        self._min_message_length = kwargs.get('min_message_length', config.MEM0_MIN_MESSAGE_LENGTH)
    ...
    def _check_memory_health(self) -> bool:
        ...
        if self._last_failure_time:
            ...
            if time_since_failure > self._retry_interval:
                logger.info(f"Memory retry interval passed ({time_since_failure:.0f}s), re-enabling memory operations")
                self._memory_enabled = True
    ...
    def _handle_memory_failure(self, operation: str, error: Exception):
        ...
        logger.warning(f"Memory {operation} failed (attempt {self._consecutive_failures}/{self._max_failures}): {error}")
        
        if self._consecutive_failures >= self._max_failures:
            self._memory_enabled = False
            self._last_failure_time = time.time()
            logger.error(f"Memory operations DISABLED after {self._max_failures} consecutive failures. Will retry in {self._retry_interval}s")
    ...
    def _handle_memory_success(self, operation: str):
        ...
        if self._consecutive_failures > 0:
            logger.info(f"Memory {operation} succeeded - resetting failure counter (was {self._consecutive_failures})")
            self._consecutive_failures = 0
    
    def _validate_and_clean_messages(self, messages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """
        ...
        for i, msg in enumerate(messages):
            ...
            if not isinstance(msg['content'], str):
                logger.warning(f"Message {i} content is not string: {type(msg['content'])} - converting")
    ...
    def _convert_context_messages_to_dict(self, context_messages: List[Any]) -> List[Dict[str, str]]:
        """
            ...
            try:
                if isinstance(msg, dict):
                    ...
                    if hasattr(msg, 'role') and hasattr(msg, 'content'):
                        ...
                        if content and str(content).strip():  # Ensure content is not empty
    ...
    async def _store_messages_async(self, messages: List[Dict[str, Any]]):
        ...
        current_user_hash = hashlib.md5(str(getattr(self, 'user_id', 'unknown')).encode()).hexdigest()
        
        # Reset counter if different user or session
        if self._user_id_hash != current_user_hash:
            logger.info(f"New user session detected, resetting message counter (user: {getattr(self, 'user_id', 'unknown')})")
        ...
        if len(cleaned_messages) < self._last_stored_message_count:
            logger.warning(f"Message count decreased from {self._last_stored_message_count} to {len(cleaned_messages)} - resetting (new session)")
            self._last_stored_message_count = 0
        
        # Additional safety: Reset if counter seems unreasonably high (corruption protection)
        if self._last_stored_message_count > len(cleaned_messages) + self.COUNTER_CORRUPTION_THRESHOLD:
            logger.error(f"Message counter corruption detected: {self._last_stored_message_count} > {len(cleaned_messages)} + {self.COUNTER_CORRUPTION_THRESHOLD} - resetting")
            self._last_stored_message_count = 0
        
        # Session timeout protection (reset after configured timeout)
        if hasattr(self, '_session_start_time') and time.time() - self._session_start_time > self._session_timeout:
            logger.info(f"Session timeout reached (>{self._session_timeout}s), resetting message counter")
            self._last_stored_message_count = 0
            self._session_start_time = time.time()
        ...
        if not new_messages:
            logger.info(f"No new messages to store (total: {len(cleaned_messages)}, last_stored: {self._last_stored_message_count})")
            return
            
        logger.info(f"Incremental storage: {len(new_messages)} new messages (out of {len(cleaned_messages)} total)")
        ...
        try:
            ...
            await loop.run_in_executor(
                None, 
                lambda: self.memory_client.add(**params)
            )
            
            end_time = datetime.datetime.now()
            duration = end_time - start_time
            logger.info(f"Successfully stored {len(messages_to_store)} NEW messages in Mem0. Duration: {duration}")
            ...
            end_time = datetime.datetime.now()
            duration = end_time - start_time
            logger.error(f"[ERROR] {end_time.isoformat()} - Error storing messages in Mem0 (async) after {duration}: {e}")
            logger.error(f"Failed params were: {params if 'params' in locals() else 'params not created'}")
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if context:
            try:
                ...
                for message in reversed(context_messages):
                    if message.get("role") == "user":
                        ...
                        if latest_user_message:
                            message_preview = latest_user_message[:50] + "..." if len(latest_user_message) > 50 else latest_user_message
                            logger.debug(f"Found latest user message: '{message_preview}'")
                            break

                if latest_user_message:
                    process_start = time.time()
                    # Enhance context with memories before passing it downstream (with fault tolerance)
                    logger.debug(f"Enhancing context with memories based on user message: '{latest_user_message[:50]}...'")
                    ...
                    if self._check_memory_health():
                        ...
                        logger.debug("Memory enhancement skipped - circuit breaker OPEN")
                    
                    # Store the conversation in Mem0 asynchronously without blocking
                    # Convert messages to dict format for storage
                    dict_messages = self._convert_context_messages_to_dict(context_messages)
                    # Create task to run in background (only if we have valid messages)
                    if dict_messages:
                        try:
                            asyncio.create_task(self._store_messages_async(dict_messages))
                        except Exception as e:
                            logger.error(f"Failed to create memory storage task: {e} - continuing without storage")
                    else:
                        logger.warning("No valid messages to store in Mem0")
                    process_end = time.time()
                    logger.debug(f"Memory operations completed in {process_end - process_start:.4f} seconds")

                # If we received an LLMMessagesFrame, create a new one with the enhanced messages
                if messages is not None:
                    logger.debug(f"Creating new LLMMessagesFrame with enhanced messages")
                ...
                end_time = datetime.datetime.now()
                duration = end_time - start_time
                logger.error(f"[ERROR] {end_time.isoformat()} - Error processing with Mem0 after {duration}: {str(e)}")
                
                # CRITICAL: Don't send error frames to user - just log and continue
                logger.info("Memory processing failed but continuing with normal conversation flow")
