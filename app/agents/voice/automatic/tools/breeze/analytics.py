...
async def _make_breeze_request(params: FunctionCallParams, operational_tab: str):
    ...
    try:
        ...
        start_time_ist = ist.localize(
            datetime.strptime(start_time_ist_str, "%Y-%m-%d %H:%M:%S")
        )
        ...
        if end_time_ist_str:
            end_time_ist = ist.localize(
                datetime.strptime(end_time_ist_str, "%Y-%m-%d %H:%M:%S")
            )
        else:
    ...
    logger.info(
        f"Requesting Breeze {operational_tab} data with payload: {json.dumps(payload)}"
    )
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(
                f"Received Breeze API response status: {response_json.get('statusCode')}"
            )
            await params.result_callback(
                response_json.get("data", {"error": "No data field in response"})
            )
