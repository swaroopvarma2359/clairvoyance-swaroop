...
from app.core.config import AUTOMATIC_ACTIONS_REQUIRE_AUTH, AUTOMATIC_WRITE_ACTIONS_AUTHORIZED_USERS
...
def filter_tools_by_authorization(
    ...
    filtered_tools = [tool for tool in tools if not is_tool_actionable(getattr(tool, 'name', None))]
    filtered_tool_functions = {name: fn for name, fn in tool_functions.items() if not is_tool_actionable(name)}

    logger.info(f"User {email} is not authorized for write tools. Filtered {len(tools) - len(filtered_tools)} tools.")
