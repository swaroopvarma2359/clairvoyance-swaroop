...
class LLMServiceWrapper:
    def __init__(self, llm_service):
        logger.debug(f"LLM Wrapper: Initializing wrapper for {type(llm_service).__name__}")
        self._llm_service = llm_service
        self._registered_functions = {}

        # Wrap the register_function method to intercept function registrations
        self._original_register_function = getattr(llm_service, 'register_function', None)
        if self._original_register_function:
            ...
            logger.warning("LLM Wrapper: LLM service does not have register_function method - HITL confirmation will not work")
                ...
                async def wrapped_function(params):
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Add success message
                            success_msg = hitl_manager.generate_success_message(name, final_args)
                            ...
                            if result_callback:
                                await result_callback({"error": f"Confirmation failed: {str(e)}"})
                            raise
                            
                    except Exception as e:
                        logger.error(f"Error in wrapped function {name}: {e}")
                        if hasattr(params, 'result_callback') and params.result_callback:
                            await params.result_callback({"error": f"Function execution failed: {str(e)}"})
                        raise
