...
class ConversationManager:
    ...
    def __init__(
        ...
        logger.info(
            f"ConversationManager initialized: max_sessions={max_sessions}, ttl={session_ttl_minutes}min"
        )
    ...
    def get_or_create_conversation(self, session_id: str) -> ConversationDebugData:
        ...
        with self._lock:
            ...
            if session_id not in self._conversations:
                ...
                logger.info(
                    f"[{session_id}] Created new conversation: {conversation_id}"
                )
    ...
    def start_turn(
        self, session_id: str, user_message: Optional[ConversationMessage] = None
    ) -> ConversationTurn:
        ...
        with self._lock:
            ...
            if len(conversation.turns) >= self.max_turns_per_session:
                ...
                logger.warning(
                    f"[{session_id}] Reached max turns ({self.max_turns_per_session}), removed oldest turn"
                )
    ...
    def add_assistant_message(
        ...
        with self._lock:
            ...
            logger.info(
                f"[{session_id}] Added assistant message to turn {conversation.current_turn.turn_number}"
            )
    ...
    def add_tool_call(
        ...
        with self._lock:
            ...
            if not conversation or not conversation.current_turn:
                logger.warning(
                    f"[{session_id}] No active turn for tool call: {function_name}"
                )
                return None

            tool_call = conversation.current_turn.add_tool_call(
                function_name, arguments, tool_call_id
            )
            logger.info(
                f"[{session_id}] Added tool call: {function_name} ({tool_call_id})"
            )
    ...
    def add_tool_result(
        ...
        with self._lock:
            ...
            if not conversation or not conversation.current_turn:
                logger.warning(
                    f"[{session_id}] No active turn for tool result: {function_name}"
                )
                return None

            tool_result = conversation.current_turn.add_tool_result(
                tool_call_id, function_name, result, success
            )
            logger.info(
                f"[{session_id}] Added tool result: {function_name} ({tool_call_id}) - {'success' if success else 'failed'}"
            )
    ...
    def complete_turn(
        ...
        with self._lock:
            ...
            logger.info(
                f"[{session_id}] Completed turn {turn.turn_number} ({status}) - duration: {turn.duration_ms:.1f}ms"
            )
    ...
    async def start_turn_with_events(
        self, session_id: str, user_content: str = "[Inferred from voice]"
        ...
        try:
            ...
            if turn:
                logger.debug(
                    f"Started conversation turn {turn.turn_number} for session {session_id}"
                )
            ...
            logger.error(
                f"Error starting conversation turn for session {session_id}: {e}"
            )
            return None

    async def add_llm_response_with_events(
        self, session_id: str, response_content: str
    ) -> Optional[ConversationEvent]:
        ...
        try:
            ...
            if assistant_message:
                ...
                if turn:
                    logger.debug(
                        f"Added LLM response to turn {turn.turn_number} for session {session_id}"
                    )
                    return ConversationEvent.turn_update(
                        session_id, turn, "llm_response"
                    )
    ...
    async def add_tool_call_with_events(
        self,
        session_id: str,
        function_name: str,
        arguments: Dict[str, Any],
        ...
        try:
            tool_call = self.add_tool_call(
                session_id, function_name, arguments, tool_call_id
            )
            ...
            if tool_call:
                ...
                if turn:
                    logger.debug(
                        f"Added tool call {function_name} to turn {turn.turn_number} for session {session_id}"
                    )
    ...
    async def add_tool_result_with_events(
        self, session_id: str, tool_call_id: str, function_name: str, result: str
        ...
        try:
            ...
            success = not (
                "error" in result_str.lower() or "failed" in result_str.lower()
            )

            tool_result = self.add_tool_result(
                session_id, tool_call_id, function_name, result_str, success
            )
            ...
            if tool_result:
                ...
                if turn:
                    ...
                    update_event = ConversationEvent.turn_update(
                        session_id, turn, "tool_result"
                    )
                    ...
                    if self._should_complete_turn(turn):
                        ...
                        if completed_turn:
                            complete_event = ConversationEvent.turn_complete(
                                session_id, completed_turn
                            )
                            events.append(complete_event)
                            logger.debug(
                                f"Completed turn {completed_turn.turn_number} for session {session_id}"
                            )

                    logger.debug(
                        f"Added tool result for {function_name} to turn {turn.turn_number} for session {session_id}"
                    )
    ...
    def _should_complete_turn(self, turn: ConversationTurn) -> bool:
        ...
        return (
            turn.assistant_response is not None
            and len(turn.tool_calls) == len(turn.tool_results)
            and len(turn.tool_calls) > 0  # Only complete if there were tool calls
        )

    def get_conversation_events(
        self, session_id: str, event_type: str = "full_state"
    ) -> List[ConversationEvent]:
...
def add_function_call(
    session_id: str, function_name: str, arguments: Dict[str, Any], tool_call_id: str
) -> Optional[ToolCall]:
...
def add_function_result(
    ...
    manager = get_conversation_manager()
    return manager.add_tool_result(
        session_id, tool_call_id, function_name, result, success
    )


def complete_conversation_turn(
    session_id: str, status: str = "completed", error: Optional[str] = None
) -> Optional[ConversationTurn]:
