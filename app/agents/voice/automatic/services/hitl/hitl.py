...
class HITLManager:
    ...
    async def request_confirmation(self, function_name: str, arguments: dict, tool_call_id: str = None) -> Dict[str, Any]:
        """
    ...
    async def _request_user_confirmation(self, confirmation_id: str, function_name: str, arguments: dict) -> dict:
        ...
        try:
            ...
            if removed:
                logger.debug(f"Cleaned up pending confirmation {confirmation_id} for function {function_name}")
    ...
    async def _send_confirmation_to_rtvi(self, payload: dict):
        ...
        try:
            ...
            if rtvi:
                ...
                logger.debug(f"Function confirmation request sent via RTVI: {payload['function_name']} with action {payload.get('arguments', {}).get('action', 'N/A')}")
            else:
    ...
    async def _wait_for_user_response(self, confirmation_id: str, timeout_seconds: int) -> dict:
        ...
        try:
            ...
            logger.debug(f"Received user response via RTVI for function {confirmation_id}: approved={response.get('approved', False) if response else False}")
            return response
        except asyncio.TimeoutError:
